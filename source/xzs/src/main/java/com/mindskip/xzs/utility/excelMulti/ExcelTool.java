package com.mindskip.xzs.utility.excelMulti;
...
public class ExcelTool<T> {
   ...
   private void init(int styleFlag){
     this.styleHead = this.workbook.createCellStyle();
       ...
       this.styleHead.setTopBorderColor(HSSFColor.HSSFColorPredefined.BLACK.getIndex());
       switch(styleFlag){
           case 1:
    ...
    * @throws Exception
    */
   public void exportExcel(List<Column> listTpamscolumn, List<T> datas, String fileName, boolean flag, boolean rowFlag, HttpServletResponse response) throws Exception{
	   splitDataToSheets(datas, listTpamscolumn,flag,rowFlag);
//	   buildExcelDocument(fileName,this.workbook,response);
	   setBrowser(response,this.workbook,fileName);
   }
   private static void setBrowser(HttpServletResponse response, HSSFWorkbook workbook, String fileName) {
       ...
       try {
           ...
           logger.info("浏览器下载成功:{}", fileName);
       } catch (Exception e) {
    	   logger.info("浏览器下载失败:{}", fileName);
           e.printStackTrace();
       } finally {
           if (outStream != null) {
               try {
            	   ...
            	   logger.error("浏览器下载失败 ::: " + e.getMessage(), e);
               }
           }
       }
		...
		try {
			...
			if (wb != null) {
				try {
					...
					logger.error("buildExcelDocument", e);
				}
			}
		}
	}
